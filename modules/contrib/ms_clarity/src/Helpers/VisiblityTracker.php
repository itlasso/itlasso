...
class VisiblityTracker {
   ...
   *
   * @return bool
   *   TRUE if the current user is being tracked by Microsoft Clarity,
   *   otherwise FALSE.
   */
  public function getUserVisibilty($account) {
    ...
    if ($this->getVisibilityRoles($account)) {
      ...
      if (($visibility_user_account_mode = $this->config->get('visibility.user_account_mode')) != 0) {
        $user_data_ms_clarity = $this->userData->get('ms_clarity', $account->id());
        if ($account->id() && isset($user_data_ms_clarity['user_account_users'])) {
          $enabled = $user_data_ms_clarity['user_account_users'];
        }
        else {
